...
$(document).ready(function () {
    $.ajax({
        url: "../ViewDashboard/gettreejsonforview",
        method: 'GET',
        async: false,
        cache: false
        ...
        if (data.treejson) {
            if (data == "<ul></ul>") {
                $("#Reporttree").html("<ul><li data-jstree='{'opened':true}'>Root node</li></ul>");

            }
            else {
                $("#Reporttree").html(data.treejson);
            }
            $("#Reporttree").jstree();
            $("#Reporttree").find(".report").parent().find('i').each(function () {
    ...
    var serverchartmdr = new Array();
    var serverpivotchartmdr = new Array();
    var serverassetkpichartmdr = new Array();

    $("#Reporttree").bind(
          "select_node.jstree", function (evt, data) {
              var ref = $('#Reporttree').jstree(true);
              var sel = ref.get_selected();
              var dashboardid = $("#" + sel.toString()).find("span").attr("id");
              var dashboardname = $("#" + sel.toString()).find("span").text();
              document.title = "Analytics -" + dashboardname;
              var pid = "Reporttree";
              var $ele = $(document.getElementById(sel.toString())).parent();
              var stext = $ele.text();
              var ptext = $ele.parent().text();
              var ttiledtext = ptext.replace(stext, "") + "/" + stext;
              $(".Toolbar").find("h1").html(ttiledtext);
              $.ajax({
                  ...
                  if (data.error) {
                      bootbox.dialog({
                          title: "Error:",
                          message: '<img src="../Analytics/Images/1398528270_196747.ico" width="100px"/><br/> <p style="color:red">Technical problems occurred while loading your dashboard try again! </p><br/>' + data.error + ''
                      });
                      ...
                      $("#widgetstore").empty();
                      $("#Dashboard").width("102%"); $("#Dashboard").attr("data-width", "102%");
    ...
    function fullscreenchart(dashboardid) {
        $(".bi-widget-item").find(".expand").click(function () {
            ...
            var ctrlclass = $(this).parent().parent().find("img").attr("class");
    ...
    $(window).resize(function () {
        ...
        $.data(this, 'resizeTimer', setTimeout(function () {
            viewchartresize(serverchartmdr, $(window).width());
    ...
    function viewchartresize(modelarr, width) {
        for (var bi = 0; bi < modelarr.length; bi++) {
            var model = modelarr[bi];
            ...
            chartstyleproperties.push({
                ...
                "chartwidth": (chartstyledata.width) - 20,
    ...
    $("#Reporttree").on('open_node.jstree', function () {
        $("#Reporttree").find(".report").parent().find('i').each(function () {
            $(this).addClass("reporticon");
        });
    });
        ...
        var template = '<div class="item-container" id="' + widgetid + '" style="margin-bottom: 5px; width:' + width + ';position: relative;float:left" data-width="' + width + '">' +
                      '<div class="item-resizable ui-resizable" style="height: auto;min-height: 198px;">' +
                          '<div class="widget" style="display: block; height: auto;min-height: 196px;">' +
                              ...
                              '<div class="widget-body" style="padding: 10px; min-height: 190px;z-index:11;"></div> </div></div> </div>';
    ...
    $("#ShareDashboardButton").click(function () {
        var ref = $('#Reporttree').jstree(true);
        var sel = ref.get_selected();
        var dashboardid = $("#" + sel.toString()).find("span").attr("id");
        var shareurl = window.location.protocol + "//" + window.location.host + "/BI360/Index?id=" + dashboardid;
        bootbox.dialog({
            title: "Dashboard Shared Link:",
            message: '<img src="/Images/1398528270_196747.ico" width="100px"/><br/> <p style="color:green">Copy This Shared URL.. </p><br/><textarea style="width:100%;height:50px;">' + shareurl + '</textarea><br/><a href="' + shareurl + '" target="_blank" >Click Here To Test</a>',
            buttons: {
                sucess: {
                    label: "Close"
                }
...
function expandmenu() {
    if ($("#ContentArea").css("left") == "0px") {
        $("#ContentArea").css("left", "240px");
    }
    else {
        $("#ContentArea").css("left", "0px");
        $("#SidebarMenuNavigationMain").css("left", "240px");
    }
    $(".bi-widget-item").find(".expand").click(function () {
        var ctrlid = $(this).parent().parent().find("img").attr("id");
        var ref = $('#Reporttree').jstree(true);
        var sel = ref.get_selected();
        var dashboardid = $("#" + sel.toString()).find("span").attr("id");
        var sFeatures = "dialogHeight: 500px;dialogWidth: 1500px;";
...
function ExportViewDashboardPDF() {


    //  alert($("#Dashboard").children().find("#widgetstore").length);
    var dashboardtxt = $(".Toolbar").find("h1").text(); alert(dashboardtxt);

    if (dashboardtxt != "Dashboard Report Viewer") {
        $.ajax({
            url: '../../Home/GeneratePDF',
            async: false,
            type: 'POST',
            data: { bidashboardhtml: $("#Dashboard").html() },
            success: function (result) {
                if (result == "ok") {
                    window.location.href = "../../Home/GetPDF?reportname=" + dashboardtxt;
                }
                else {
                    alert(result);
                    return false;
