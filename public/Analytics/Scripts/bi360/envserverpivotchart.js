function envserverpivotchart($scope, $location, $http, $compile, left, top, dropeditem, controlid, target, type) {
    ...
    if (controlid == "new")//Creating new Control
    {
        bipivotchart.add([{
            ...
            yaxisproperties: { "yname": "Untitled", "yformatas": "Text", "ydecimalval": "0", "ytitlecolor": "#0000ff", "ytitlestyle": "Arial", "ytitlefontsize": "10", "yinterval": "yauto", "ymanualinterval": "", "yPrefix": "", "ySuffix": "", "yShowlabels": "true", "ylblstyleangle": "0", "yaxistitle": "", "ylabeldrop": "1", "yshowgridlines": "true" },
            legendproperties: { "PrimaryBackColor": "#ffffff", "SecondaryBackColor": "#ffffff", "legendforecolor": "#ffffff", "legenddock": "Bottom", "legendGradient": "None", "Hatching": "Cross", "BorderColor": "#000000", "BorderSize": "2", "BorderDashStyle": "Solid", "ShadowOffset": "0", "LegendFontSize": "10", "LegendFontStyle": "Arial", "showlegends": "true" },
        ...
        selecteditem = bipivotchart.byId(bipivotchart, Controlid);
    }
    else {
        selecteditem = bipivotchart.byId(bipivotchart, $scope.view.getSelected().controlid);
        if (selecteditem.get("datatabs").length > 0)
            $scope.datatabs = selecteditem.get("datatabs");
    }
    //$("#seriespivotcolorpick").colorpicker();
    ...
    selecteditem = bipivotchart.byId(bipivotchart, $scope.view.getSelected().controlid);
    $("#seriespivotpropertiestab").hide(); $("#drilldownpivotproptab").hide();

    // ......... For loading html template with preview and properties tree.................//

    $http.get('../Analytics/Bi360Templates/Tabs/PivotChart.html').success(function (t) {
        ...
        $(".ctype").click(function () {
            ...
            if ((charttype == "Pie") || (charttype == "StackedColumn") || (charttype == "StackedColumn100") || (charttype == "StackedBar") || (charttype == "StackedBar100")) {
                $("#piechartprop").show();
                $("#pielabelstyle").show();
                $("#ladeldatashow").show();
                $("#drawingstyle").show();
                ...
                // $('#seriespivotcolorpick').colorpicker("destroy");
                if (series.series_color.length > 0) {
                    $("#srcolorpick").html(' <div style="float:left;width:30px"><input id="seriespivotcolorpick" value="' + series.series_color[0] + '" style="display:none" /> <span style="width:4px;"></span></div>')
                    $("#seriespivotcolorpick").colorpicker({ showOn: 'button' });
                    //series.series_color = series.series_color[0];
                    $scope.updateseriescolorevent();

                }
                else {
                    $("#srcolorpick").html(' <div style="float:left;width:30px"><input id="seriespivotcolorpick" value="' + piecolors[indexselected] + '" style="display:none" /> <span style="width:4px;"></span></div>')
                    $("#seriespivotcolorpick").colorpicker({ showOn: 'button' });
                    series.series_color = piecolors[indexselected];
                    $scope.updateseriescolorevent();

                }
                $("#seriespivotcolorpick").parent().width(30);
            }
            else if ((charttype == "StackedArea") || (charttype == "StackedArea100") || (charttype == "Polar") || (charttype == "Range") || (charttype == "RangeBar") || (charttype == "RangeColumn") || (charttype == "Point")) {
                $("#piechartprop").show();
                $("#pielabelstyle").show();
                $("#ladeldatashow").show();
                $("#drawingstyle").hide();
        ...
        $("#pivotseriessvalues").unbind("change");
        $("#pivotseriessvalues").on("change", function () {
            var arrdata = selecteditem.get("series");
            var ref = $('#bitree').jstree(true); var sid = ref.get_selected();
            var series = _.find(arrdata, function (rw, index) { indexselected = index; return rw.id == sid; });

            if ($('#pivotseriessvalues').prop('checked')) {
        ...
        var data = [
                  ...
                  { "id": "Title", "parent": "Pivot", "text": "Chart Title", 'state': { 'opened': true } },
                   { "id": "TitleFormula", "parent": "Title", "text": "Title Formula" },
                   { "id": "Area", "parent": "Pivot", "text": "Chart Area" },
        ...
        $('#bitree').bind('loaded.jstree', function (e, data) {
            ...
            $("#pivotchartcolorpickbg").find(".colorPicker-picker").css({ "background-color": chartpropobjobj1.backgroundcolor });
            $("#chartbrcolor").val(chartpropobjobj1.bordercolor);
            $("#pivotchartcolorpickbr").find(".colorPicker-picker").css({ "background-color": chartpropobjobj1.bordercolor });
            $("#pivotchartborderwidth").val(chartpropobjobj1.borderwidth);
            $("#chartborderstyle").val(chartpropobjobj1.borderstyle);
            $("#pivotchartGGradiant").val(chartpropobjobj1.backgradientstyle);
            $("#pivotchartGFrame").val(chartpropobjobj1.borderskin);
            ...
            $("#legendforecolor").val(chartlegendpropobj.legendforecolor);
            $("#clegendforecolor").find(".colorPicker-picker").css({ "background-color": chartlegendpropobj.legendforecolor });
            ...
            if (chartxpropobj.zTotals == "true") {
                $("#zTotals").prop('checked', true);
            }
            else {
                $("#zTotals").prop('checked', false);
            }
            if (chartxpropobj.zAverages == "true") {
                $("#zAverages").prop('checked', true);
            }
            else {
                $("#zAverages").prop('checked', false);
            }
            ...
            var chartypropobj = selecteditem.get("yaxisproperties");
            $("#ychrtPrefix").val(chartypropobj.yPrefix);
            $("#ychrtSuffix").val(chartypropobj.ySuffix);
            ...
            $("#ylblstyleangle").val(chartypropobj.ylblstyleangle);
            $("#yfmtas").val(chartypropobj.yformatas);

            if (chartypropobj.yformatas == "Number") {
                $("#yfmtdecimaldiv").show();
            } else {
                $("#yfmtdecimaldiv").hide();
            }
            $("#ydecimalvalue").val(chartypropobj.ydecimalval);
            //...............initial chart y-axis properties ends........................//



            $("#serverpivotchartdatatab").hide(); $("#serverpivotchartproperiestab").show(); $("#seriespivotpropertiestab").hide(); $("#VFormula-menu").hide(); $("#vf-formula-bar").hide();
            $("#pivotcharttabprop").show(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide();
            $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriescontentproperties").hide(); $("#drilldownpivotproptab").hide();
        ...
        "select_node.jstree", function (evt, data) {
            ...
            if (sel.toString().indexOf("_") != -1) {
                ...
                if (_parentid) {
                    ...
                    $("#serverpivotchartdatatab").show(); $("#serverpivotchartproperiestab").show(); $("#seriespivotpropertiestab").show(); $("#xaxistabproperties").hide();
                    $("#pivotcharttabprop").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide(); $("#drilldownpivotproptab").hide();
                    ...
                    if (series.showvaluesonlabel == "true") {
                        $("#pivotseriessvalues").prop('checked', true);
                    }
                    else {
                        $("#pivotseriessvalues").prop('checked', false);
                    }
                    ...
                    $(document.getElementById("serverpivotchartdatatab")).click();
                    $("#serverpivotchartdatatab").show(); $("#seriespivotpropertiestab").hide(); $("#drilldownpivotproptab").hide();
                    ...
                    $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                    $("#serverpivotchartproperiestab").show();
                    $("#pivotcharttabprop").hide(); $("#xaxistabproperties").show(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide();
                    $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriescontentproperties").hide();
                    $("#drilldownpivotproptab").hide();
                    var chartxpropobj = selecteditem.get("xaxisproperties");
                    $("#xchrtPrefix").val(chartxpropobj.xPrefix);
                    $("#xchrtSuffix").val(chartxpropobj.xSuffix);
                    if (chartxpropobj.xShowlabels == "true")
                    ...
                    $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                    $("#serverpivotchartproperiestab").show();
                    $("#pivotcharttabprop").hide(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").show(); $("#charttitleprop").hide();
                    $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriescontentproperties").hide();
                    $("#drilldownpivotproptab").hide();
                ...
                $scope.selecteditem = "Pivot";
                $scope.selected = 2;
                $("#serverpivotchartdatatab").hide(); $("#serverpivotchartproperiestab").show(); $("#VFormula-menu").hide(); $("#vf-formula-bar").hide();
                $("#pivotcharttabprop").show(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#pivotcharttitleprop").hide();
                $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriespivotpropertiestab").hide(); $("#seriescontentproperties").hide();
                $("#drilldownpivotproptab").hide();
                ...
                $("#pivotchartcolorpickbg").find(".colorPicker-picker").css({ "background-color": chartpropobjobj.backgroundcolor });
                $("#chartbrcolor").val(chartpropobjobj.bordercolor);
                $("#pivotchartcolorpickbr").find(".colorPicker-picker").css({ "background-color": chartpropobjobj.bordercolor });
                $("#pivotchartborderwidth").val(chartpropobjobj.borderwidth);
                $("#chartborderstyle").val(chartpropobjobj.borderstyle);
                $("#pivotchartGGradiant").val(chartpropobjobj.backgradientstyle);
                $("#pivotchartGFrame").val(chartpropobjobj.borderskin);
            }
            else if (sel == "Charttype") {
                $scope.selecteditem = "Charttype"; $("#seriespivotpropertiestab").hide(); $("#drilldownpivotproptab").hide();
                ...
                if (_parentid) {
                    ...
                    //$scope.selected = 1;               
                    $scope.selecteditem = "ZAXIS"; $("#serverpivotchartdatatab").show(); $("#VFormula-menu").show(); $("#vf-formula-bar").show();
                    $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                    $(".imgvalidate").click(function () {
                        var arrdata = selecteditem.get("series");
                        _.find(arrdata, function (rw, index) {
                    ...
                    $scope.selected = 1; $("#serverpivotchartdatatab").show(); $("#serverpivotchartproperiestab").show(); $("#seriespivotpropertiestab").show(); $("#drilldownpivotproptab").hide();
                    $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                ...
                $scope.selected = 2; $scope.selecteditem = "YAXIS"; $("#VFormula-menu").show(); $("#vf-formula-bar").show();
                $("#serverpivotchartdatatab").hide(); $("#serverpivotchartproperiestab").show();
                $("#pivotcharttabprop").hide(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").show();
                $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#chartareaprop").hide(); $("#seriespivotpropertiestab").hide();
                $("#seriescontentproperties").hide(); $("#drilldownpivotproptab").hide();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                ...
                $scope.selected = 1; $("#serverpivotchartdatatab").show(); $("#serverpivotchartproperiestab").hide(); $("#seriespivotpropertiestab").hide(); $("#drilldownpivotproptab").hide();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                $("#VFormula-menu").show(); $("#vf-formula-bar").show();
                ...
                $("#chartareaprop").show(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriespivotpropertiestab").hide();
                $("#seriescontentproperties").hide(); $("#drilldownpivotproptab").hide(); $("#pivotcharttabprop").hide();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                ...
                $("#serverpivotchartdatatab").hide(); $("#serverpivotchartproperiestab").show();
                $("#charttabprop").hide(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide();
                $("#chartareaprop").hide(); $("#chartlegendprop").show(); $("#seriescharttypeprop").hide(); $("#seriespivotpropertiestab").hide();
                $("#seriescontentproperties").hide(); $("#drilldownpivotproptab").hide(); $("#pivotcharttabprop").hide();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                ...
                $("#serverpivotchartdatatab").show(); $("#serverpivotchartproperiestab").hide();
                $("#pivotcharttabprop").hide(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide();
                $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriescontentproperties").hide(); $("#drilldownpivotproptab").hide();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
            }
            else if (sel == "Drilldown") {
                $scope.selected = 4; $scope.selecteditem = "Drilldown"; $("#VFormula-menu").show(); $("#vf-formula-bar").show();
                $("#serverpivotchartdatatab").hide(); $("#serverpivotchartproperiestab").hide(); $("#seriespivotpropertiestab").hide();
                $("#charttabprop").hide(); $("#xaxistabproperties").hide(); $("#yaxistabproperties").hide(); $("#charttitleprop").hide();
                $("#chartareaprop").hide(); $("#chartlegendprop").hide(); $("#seriescharttypeprop").hide(); $("#seriescontentproperties").hide();
                $("#drilldownpivotproptab").show(); $("#drilldownpropertab").show();
                $scope.modal.tablestatus = true; $scope.modal.expressionstatus = true; $scope.modal.strnumstatus = true; $scope.modal.variablestatus = true;
                ...
                $(".reqparamtext_chartx").val(drilldownObj.ChartXParameterValue);
        ...
        $scope.getconnectiondetails = function (sid) {
            ...
            if (charttype == "pie") {
                ...
                if (sinfo.series_color.length > 0) {
                    $("#srcolorpick").html(' <div style="float:left;width:30px"><input id="seriespivotcolorpick" value="' + sinfo.series_color[0] + '" style="display:none" /> <span style="width:4px;"></span></div>')
                    $("#seriespivotcolorpick").colorpicker({ showOn: 'button' }); $scope.updateseriescolorevent();
                }
                else {
                    $("#srcolorpick").html(' <div style="float:left;width:30px"><input id="seriespivotcolorpick" value="' + piecolors[indexselected] + '" style="display:none" /> <span style="width:4px;"></span></div>')
                    $("#seriespivotcolorpick").colorpicker({ showOn: 'button' }); $scope.updateseriescolorevent();
            ...
            $("#chartpivotGpontWidth").val(sinfo.pointwidth);
            $("#pivotbubbleselstyle").val(sinfo.pivotbubbleselstyle);
            $("#stackedcharttypes").val(sinfo.stackedstyle);
            $("#srpivotchartlinewidth").val(sinfo.srlinewidth);
        ...
        $scope.updateseriescolorevent = function () {
            $("#seriespivotcolorpick").on("change.color", function (event, color) {
                var colorarry = [];
        ...
        $scope.updatedbconnections = function () {
            ...
            if ($scope.selecteditem == "ZAXIS") {
                ...
                if ((series.charttype != "pie") || (series.charttype != "doughnut") || (series.charttype != "pyramid") || (series.charttype != "funnel")) {
                    var carray = [$("#seriespivotcolorpick").colorpicker("val")];
                ...
                var selecteditem = bipivotchart.byId(bipivotchart, $scope.view.getSelected().controlid);
                var selecteddatatab = $(".tabstriplist").find("[data-selected='true']").attr("id");
        ...
        $scope.updatechartproperties = function () {
            ...
            updatechartprop.borderwidth = $("#pivotchartborderwidth").val();
            updatechartprop.borderstyle = $("#chartborderstyle").val();
            updatechartprop.backgradientstyle = $("#pivotchartGGradiant").val();
            updatechartprop.borderskin = $("#pivotchartGFrame").val();
            ...
            updatechartlegendprop.legendforecolor = $("#legendforecolor").val();
        ...
        $scope.updatecharttypeprop = function () {
            ...
            series.pointwidth = $("#chartpivotGpontWidth").val();
            series.pivotbubbleselstyle = $("#pivotbubbleselstyle").val();
            series.stackedstyle = $("#stackedcharttypes").val();
            series.srlinewidth = $("#srpivotchartlinewidth").val();
