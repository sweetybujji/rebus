...
function envmultiselect($scope, $location, $http, $compile, left, top, dropeditem, controlid, target, type) {
    ...
    if (controlid == "new")//Creating new Control
    {
        ...
        selecteditem = multiselect.byId(multiselect, Controlid);
    }
    else {
        selecteditem = multiselect.byId(multiselect, $scope.view.getSelected().controlid);
    }
    $scope.selecteditem = "Dropdown";
    //loads Dropdown properties html page
    $http.get('../Analytics/Bi360Templates/Tabs/MultiSelect.html').success(function (t) {
        //bind tabs html to tabs div
        //$("#" + $scope.view.getSelected().controlid + "").find(".inputdropdown").multipleSelect();
    ...
    if (controlid != "new") {
        Controlid = controlid;
        var selecteditem = multiselect.byId(multiselect, $scope.view.getSelected().controlid);
