...
function fn_Menubar_Toggle() {
    ...
    $("#ace-settings-hover").click(function () {
        if ($(this).is(':checked')) {
            ...
            //  alert("Unchecked");
            var sidemenuHover = "Hoversidemenu";
            var mydata = JSON.stringify({ sidemenuHover: sidemenuHover });
            $.ajax({
                url: "../Sidebar_Changes_Submenu",
                type: "POST",
...
function RedirectToselectedEForm(_tagid) {
    localStorage.setItem("FV_TagId", _tagid);
    EFormsScope.go("FormViewer", {
        "type": null,
        "_tagid": _tagid,
        "KeyId": null,
        "KeyValue": null,
